...
当 exchange constant（交换常数 / 交換定数）`J > 0` 时，相邻自旋反平行会降低能量；三角形上三个连续可转动的经典自旋选择彼此约 `120 degree`，因为这比强行做两个反平行、一个冲突更公平地分摊挫折。
...
对两根长度固定的经典向量，`S_i · S_j = |S|^2 cos(theta)`。`J > 0` 时，夹角 `theta = 180 degree` 的一对边能量最低。但三角形不可能让三对同时都是 `180 degree`。

## 为什么经典三角形选 120 度

设三个自旋长度相同。如果它们依次相差 `120 degree`，三条边内积都是 `cos(120 degree) = -1/2`。没有任何一条边完全满足反平行，但每条边都同等降低了能量。
...
Kagome lattice（Kagome 格子 / カゴメ格子）由角共享三角形组成。每个局域三角形都想满足 `S_A + S_B + S_C = 0`，但相邻三角形共享自旋。这会留下许多能量相同或接近的经典构型，而不是迅速选出唯一的 Néel order（尼尔有序 / ネール秩序）。
...
练习：请用一句话分别解释 `J > 0`、`120 degree`、`没有长程有序` 三者之间是什么关系，又不是什么关系。
