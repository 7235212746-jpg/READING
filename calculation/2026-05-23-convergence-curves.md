...
\\[
\Delta E = E(q=0) - E(\mathrm{FM})
\\]

或者：

\\[
\Delta E = E(\sqrt{3}\times\sqrt{3}) - E(q=0)
\\]
...
4. 画出 \\(\Delta E\\) 随 `ENCUT` 的曲线，同时记录磁矩是否跳变。
...
- `ENCUT` 对 \\(E_{\mathrm{FM}}\\)、\\(E_{q=0}\\)、\\(\Delta E\\) 与 magnetic moments 的记录。
- k mesh 对 \\(E_{\mathrm{FM}}\\)、\\(E_{q=0}\\)、\\(\Delta E\\) 与 magnetic moments 的记录。
...
练习：论文报告 \\(q=0\\) 比 FM 低 \\(0.7\ \mathrm{meV/formula\ unit}\\)，但没有给 \\(\Delta E\\) 随 k 点的曲线。你在笔记里应把这条结论标成什么风险等级，为什么？
