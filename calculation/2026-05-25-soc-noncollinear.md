...
这正是 magnetic anisotropy（磁各向异性 / 磁気異方性）、拓扑能带开隙和某些 Kagome 磁性问题的重要入口。可怕的是，这些能量差可能非常小；官方 VASP 文档明确警告 SOC 的能量差可在 `micro-eV/atom` 量级，k 点收敛困难且计算耗时很大。
...
Kagome 三角形的 `120 degree` 构型本来就要求 noncollinear magnetism（非共线磁性 / 非共線磁性）。在不加 SOC 的理想计算中，你可以研究自旋之间的相对角度和能量。
...
```text
MAE = E(direction A) - E(direction B)
```
...
练习：某论文声称面内方向比面外低 `3 micro-eV/atom`，却只使用一个 k 网格。请列出你会要求查看的三张收敛/验证图。
