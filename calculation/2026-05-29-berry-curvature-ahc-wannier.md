...
今天的计算问题是：论文里说“我们计算了 Berry curvature，因此解释了反常霍尔效应”，这句话需要哪些中间步骤才可信？

最短答案是：先用 DFT 得到能带和波函数；再用 Wannier 函数把复杂能带转成可在超密 k 网格上快速插值的低能模型；然后在很多 k 点上计算 Berry curvature；最后把被占据能带对 Berry curvature 的贡献积分，得到内禀反常霍尔电导 AHC 的估计 [R1, R2, R3, R4]。
...
| Berry curvature | 每个 k 点附近波函数几何变化的量 | 热点是否靠近 EF，是否来自可信的能带交叉/开缝 |
| AHC | anomalous Hall conductivity，反常霍尔电导 | 单位、符号、方向、温度和费米能级假设是否说明 |
| k 网格 | 在动量空间采样的点阵 | 是否足够密，热点附近是否自适应加密 |
...
```mermaid
flowchart TD
  structure["晶体结构、磁结构、元素赝势"] --> dft["DFT 自洽计算：得到电荷密度和能带"]
  dft --> soc["加入 SOC 和非共线磁性设置，若材料需要"]
  soc --> wannier["选择投影轨道与能量窗口，构造 Wannier 模型"]
  wannier --> check["验收：Wannier 能带是否贴住 DFT 能带"]
  check --> berry["在密 k 网格上计算 Berry curvature"]
  berry --> ahc["积分得到内禀 AHC 随 EF 或温度的变化"]
  ahc --> compare["与实验 AHE、磁化、纵向电阻和 ARPES 对照"]
```

这里的关键不是“软件跑通”，而是每一步是否留下可审查证据。Berry curvature 对局部能带细节非常敏感，一处错误投影或没收敛的小能隙就可能让图上出现假热点。
...
| SOC 设置 | SOC 常打开 Dirac/Weyl 附近小能隙 | 不开 SOC 可能漏掉关键 Berry curvature |
| 赝势/PAW 数据 | 影响能带和轨道成分 | 不同赝势可能改变细节 |
...
2. 颜色代表 Berry curvature 的哪个分量？单位是什么？
3. 是单条能带的 Berry curvature，还是所有占据能带求和？
4. 热点是否靠近 EF？如果 EF 移动 10 meV、50 meV，结果变不变？
5. 热点是否来自 SOC 打开的能隙、Weyl 点附近、或带反交叉？
6. k 网格是否足够密，是否使用自适应加密？

这个读法来自 AHC 的物理性质：AHC 不是看某一点，而是对整个 Brillouin zone 中被占据态的 Berry curvature 积分 [R1, R2, R3]。
...
| 材料类型 | 关键检查 | 为什么 |
| --- | --- | --- |
| Fe3Sn2 | 铁磁方向、SOC、Dirac 带、薄膜/块体差异 | AHE 和 Nernst 响应可能受磁各向异性与能带几何共同影响 [R6, R7] |
